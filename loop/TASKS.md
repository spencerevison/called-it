...
- [ ] T55 [risk:low]: Migration — add nullable `option text` to `premortems` (the option string this pre-mortem targets; `NULL` = legacy/whole-decision pre-mortem). Index `(decision_id, option)`. — AC: migration applies to a fresh **and** a seeded db; existing premortem rows read back with `option = null`; `pnpm db:types` regenerated.
