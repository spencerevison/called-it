...
- [ ] T09 [risk:low]: Migration: `checkins` (incl. `overall_attribution`, the completed-requires-attribution CHECK, and the `(decision_id, horizon) where horizon <> 'custom'` unique index), `checkin_failures` + indexes; add the deferred `forecasts.resolved_in_checkin_id` FK now that `checkins` exists. — AC: `linked_risk_id` FK nullable and enforced; completing a check-in without `overall_attribution` is rejected by the DB; an insert with a bogus `resolved_in_checkin_id` is rejected (proves the deferred FK was actually added).
- [ ] T10 [risk:low]: Migration: `prompt_versions`, `judge_scores` (incl. `input_hash`), `eval_items`, `eval_runs`; add the deferred `premortems.prompt_version` FK now that `prompt_versions` exists. — AC: `judge_scores.prompt_version` and `premortems.prompt_version` FKs enforced.
- [ ] T11 [risk:high]: RLS on all user tables: **select** policies where `user_id = auth.uid()`; **no authenticated insert/update/delete policies** (writes are service-role only, DATA_MODEL rule 0); `prompt_versions` read-authenticated; **eval tables `enable row level security` with zero policies** (deny-all, not RLS-disabled). — AC: policy tests in `pnpm test:db` (requires `supabase start`; documented; NOT part of `pnpm check`/CI) — anon reads zero rows; an authenticated user reads zero eval rows and **cannot write any user table directly** (insert/update via the anon+JWT client is rejected by RLS); user A cannot read user B's rows; the server-action layer rejects a child row whose parent belongs to another user or another decision.
...
- [ ] T25 [risk:high]: Pre-mortem review UI: risk list grouped by category with severity badges; add-own-risk (source=user, **draft only** — risks are judge input, frozen at commit); regenerate allowed on draft only. — AC: user risks persist and render distinctly; add-own-risk blocked once active; ownership verified before insert (historical IDOR — docs/EVIDENCE.md).
...
- [ ] T34 [risk:high]: Check-in flow step 3–4: failures (description, was_knowable, link to a pre-mortem risk via picker or "unlisted", attribution per failure); required `overall_attribution`; complete check-in. — AC: completing without overall_attribution blocked; links persist; a `linked_risk_id` must belong to the same decision as the check-in (DATA_MODEL rule 4) — a cross-decision link is rejected by the action.
- [ ] T35 [risk:high]: Resolution: from any check-in (or decision page), mark decision resolved/abandoned → `resolved_at`, terminal event, remaining pending/due check-ins set to skipped (DATA_MODEL rule 5). — AC: skipped check-ins never turn due (T29/T30 respect terminal states).
