...
- [x] T32 [DEFER] [risk:high]: Email notification on due (Resend), one per transition. — AC: template renders decision title + link; no duplicate sends on cron re-runs.
...
- [x] T37 [risk:low]: Prompt registry: parse `prompts/*.md` frontmatter-ish headers, register into `prompt_versions` with content hash at startup/build; drift (file vs hash) fails loudly. — AC: unit tests for parse + drift detection.
- [x] T38 [risk:high]: Judge server action at commit: assemble outcome-blind input per DATA_MODEL integrity rule 3 (assertion test greps payload for outcome-field names), call judge_v1, validate JSON, persist judge_scores (incl. `input_hash` = hash of the assembled input) + Langfuse trace. — AC: mocked tests incl. contamination=true logging path; a test asserts the stored input_hash matches a re-hash of the asserted-outcome-free payload. — WATCH: fat task; run whole at the raised turn cap, split only on capability failure (not turn-budget failure).
- [x] T39 [risk:low]: Judge display on decision detail: three scores with anchor tooltips, per-dimension rationale, evidence spans, "experimental" badge gated by an env/config flag (flipped when EVAL_PLAN §4 bar is met). — AC: badge copy explains why, in one sentence.
...
- [x] T40 [risk:low]: `pnpm eval:import` — validate goldset/*.json against the schema (zod), load into eval_items; clear errors with file+path. — AC: example-001 and fixtures import (schema ignores keys prefixed with `_`, e.g. `_note`); a deliberately broken fixture fails with a useful message.
- [x] T41 [risk:high]: `pnpm eval:judge --version <v>` — outcome-blind judging of eval items, agreement math (within1/exact/MAE per dimension + macro), disagreement-case detail (both rationales) to stdout + gitignored `docs/eval/detail/`, committed report (aggregate metrics + item ids ONLY — EVAL_PLAN privacy rule) to `docs/eval/`, `eval_runs` row. — AC: agreement math unit-tested on a small hand-computed fixture set (3 items → known within1/MAE); a test asserts the committed report contains no `decision`/`rationale` content from items.
...
- [x] T46 [risk:low] (deps: T20): Dashboard shell: summary cards (decisions, resolved, open check-ins, Brier headline) with min-n states. — AC: matches seed hand-values.
...
- [ ] T53 [BLOCKED: requires operator-held Vercel/Trigger.dev/Langfuse account access and interactive CLI login — no credentials or CLIs available to the author agent] [risk:high]: Vercel deploy (envs documented), Trigger.dev deployed, Langfuse verified in prod. — AC: production URL live end-to-end with a real decision entry.
- [ ] T54 [DEFER] [BLOCKED: requires a live production deployment (T53, itself blocked) plus real accumulated usage data (Brier score, bias finding, judge agreement %) and a narrated screen recording — none of which an author agent can produce] [risk:low]: Record the 60s demo per EVAL_PLAN §6. — AC: link in README.

## P10 · Option compare (post-v1)

*(Flavor A: a pre-mortem **per option** so the user can compare failure modes across the options before committing — the "I want to see stay-vs-move side by side" case. Invariant that must not bend: only the **chosen** option's risks are judge input; the other options' pre-mortems are decision-time context, never judge input. Outcome-blindness and the judge-input freeze (DATA_MODEL rule 1/3) are preserved by construction. Back-compat: a decision that never uses compare keeps a single whole-decision pre-mortem, stored with `option = null`, and behaves exactly as today.)*

- [x] T55 [risk:low]: Migration — add nullable `option text` to `premortems` (the option string this pre-mortem targets; `NULL` = legacy/whole-decision pre-mortem). Index `(decision_id, option)`. — AC: migration applies to a fresh **and** a seeded db; existing premortem rows read back with `option = null`; `pnpm db:types` regenerated.
- [x] T56 [risk:high] (deps: T55): Per-option generation — `generatePremortem(decisionId, option)` renders the prompt with `chosen_option = option` (imagines *that* path failing) and persists the premortem with its `option`; regenerate replaces the latest premortem **for that option only**, and T25's user-risk carry-forward stays scoped to the same option. Draft-only; `option` must be one of `options_considered` (else rejected); ownership verified before insert (as T24/T25). Omitting `option` preserves today's whole-decision behavior (`option = null`). — AC: mocked tests — an `option` not in `options_considered` is rejected; regenerating option A leaves option B's risks and the null-option pre-mortem untouched; user-risk carry-forward never crosses options.
- [x] T57 [risk:low] (deps: T56): Option-compare UI on the draft edit page — each option as its own section with generate/regenerate + add-own-risk, risks grouped by category with severity badges; the chosen option is visually marked. Draft-only (frozen after commit). — AC: per-option risks render distinctly; generate/regenerate/add-risk each scoped to the right option; add-own-risk blocked once active; labels/focus consistent with T25 + T52 (axe clean).
- [x] T58 [risk:high] (deps: T56): Judge-input scoping at commit — the judge-input assembly (DATA_MODEL rule 3 / T38) selects **only the chosen option's** pre-mortem risks (treat `option = null` as the chosen path so legacy/single-option decisions are unchanged); non-chosen options' pre-mortems are retained as the decision-time record but never enter the judge payload or `input_hash`. The commit freeze (rule 1) covers only the chosen option's risks. — AC: assertion test — the assembled judge payload contains only chosen-option risks; `input_hash` is unchanged by adding or regenerating a **non-chosen** option's pre-mortem; the rule-3 outcome-field grep still passes; a decision with only a null-option pre-mortem hashes identically to pre-P10 (no regression). — WATCH: judge-input-integrity path (same class as T38); a high-severity finding here should HALT for a human, not go to the fixer.

> **QUALITY GATE — P10:** Fable reviews the cumulative diff since the last gate. (Details: CLAUDE.md.)
