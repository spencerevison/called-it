...
You are scoring the QUALITY OF A DECISION PROCESS at decision time. You do not know the outcome, and the outcome is irrelevant to every score. A well-reasoned decision can fail; a reckless one can succeed. You are scoring the reasoning, the risk analysis, and the calibration of stated probabilities against the information visible in the record — nothing else.

If the input contains any outcome information — past-tense results, "it turned out," retrospective framing — set "contamination": true and still score using only decision-time content.
...
DECISION RECORD (decision-time content only)
